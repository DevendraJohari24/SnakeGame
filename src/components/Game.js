...
class Game extends Component{
    ...
    gameTick = () => {
        ...
        var newx = (this.state.snakeList[0][0] + newInc[0]), newy = (this.state.snakeList[0][1] + newInc[1]);
        if(this.state.snakeList.filter((i) => {return i[0]==newx%19 && i[i]==newy%19}).length || newx<0 || newx>18 || newy<0 || newy>18){
    ...
    renderGameMatrix = () => {
        return this.state.gameMatrix.map((row) => {
            return row.map((t) => {
                if(t===2) return <Tile color={this.state.isFoodVisible? "red": "lightgray"} />
    ...
    restartGame = () => {
        this.setState({gameOver: false, snakeList: [[1,1], [1, 2], [2, 2]], increment: [1, 0]});
    }
        ...
        return (
            <>
                <div className="game-container">
                    {
                        this.state.gameOver ? <><p>Game Over</p><button onClick={this.restartGame}>Restart</button></>: this.renderGameMatrix()
                    }
